...
class AllocationRequest(BaseModel):
    vm_id: str
    algorithm: str = "weighted"  # Valeur par défaut

class IPAddresses(Base):
    __tablename__ = "ip_addresses"
    ip_address = Column(String, primary_key=True)
    is_used = Column(Boolean, default=False)
    vm_id = Column(int, nullable=True, autoIncrement=True)
...
async def allocate_vm(request: AllocationRequest, db: Session = Depends(get_db)):
    vm_id = request.vm_id
